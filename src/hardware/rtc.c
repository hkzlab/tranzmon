...
static __sfr __at (RTC_BASE+0x03) CLK_Reg_3;
static __sfr __at (RTC_BASE+0x04) CLK_Reg_4;
static __sfr __at (RTC_BASE+0x05) CLK_Reg_5;
static __sfr __at (RTC_BASE+0x06) CLK_Reg_6;
static __sfr __at (RTC_BASE+0x07) CLK_Reg_7;
static __sfr __at (RTC_BASE+0x08) CLK_Reg_8;
static __sfr __at (RTC_BASE+0x09) CLK_Reg_9;
static __sfr __at (RTC_BASE+0x0A) CLK_Reg_A;
static __sfr __at (RTC_BASE+0x0B) CLK_Reg_B;
static __sfr __at (RTC_BASE+0x0C) CLK_Reg_C;
static __sfr __at (RTC_BASE+0x0D) CLK_Reg_D;
static __sfr __at (RTC_BASE+0x0E) CLK_Reg_E;
static __sfr __at (RTC_BASE+0x0F) CLK_Reg_F;

static uint8_t raw_rreg(uint8_t reg);
static void raw_wreg(uint8_t reg, uint8_t val);
static uint8_t read_rtc_reg(uint8_t reg);
static void write_rtc_reg(uint8_t reg, uint8_t val);
...
void rtc_get(rtc_stat *cs) {
    // Seconds
    cs->s = read_rtc_reg(0x00) & 0x0F;
    cs->s |= (read_rtc_reg(0x01) & 0x07) << 4;
    
    // Minutes
    cs->m = read_rtc_reg(0x02) & 0x0F;
    cs->m |= (read_rtc_reg(0x03) & 0x07) << 4;
    
    // Hours
    cs->h = read_rtc_reg(0x04) & 0x0F;
    cs->h |= (read_rtc_reg(0x05) & 0x03) << 4;
    
    // Day
    cs->d = read_rtc_reg(0x06) & 0x0F;
    cs->d |= (read_rtc_reg(0x07) & 0x03) << 4;   
    
    // Month
    cs->M = read_rtc_reg(0x08) & 0x0F;
    cs->M |= (read_rtc_reg(0x09) & 0x01) << 4;
        
    // Year
    cs->y = read_rtc_reg(0x0A) & 0x0F;
    cs->y |= (read_rtc_reg(0x0B) & 0x0F) << 4;
  
    // DOW
    cs->dow = read_rtc_reg(0x0C) & 0x07;
}

void rtc_set(rtc_stat *cs) {
    // Seconds
    write_rtc_reg(0x00, cs->s & 0x0F);
    write_rtc_reg(0x01, (cs->s >> 4) & 0x07);
    
    // Minutes
    write_rtc_reg(0x02, cs->m & 0x0F);
    write_rtc_reg(0x03, (cs->m >> 4) & 0x07);
    
    // Hours
    write_rtc_reg(0x04, cs->h & 0x0F);
    write_rtc_reg(0x05, (cs->h >> 4) & 0x03);
    
    // Day
    write_rtc_reg(0x06, cs->d & 0x0F);
    write_rtc_reg(0x07, (cs->d >> 4) & 0x03);
    
    // Month
    write_rtc_reg(0x08, cs->M & 0x0F);
    write_rtc_reg(0x09, (cs->M >> 4) & 0x01);
    
    // Year
    write_rtc_reg(0x0A, cs->y & 0x0F);
    write_rtc_reg(0x0B, (cs->y >> 4) & 0x0F);
